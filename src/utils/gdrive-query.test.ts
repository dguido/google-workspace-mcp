...
describe("utils/gdrive-query", () => {
  describe("escapeQueryString", () => {
    ...
    it("escapes both backslashes and single quotes", () => {
      expect(escapeQueryString("path\\to\\file's name")).toBe("path\\\\to\\\\file\\'s name");
    });
  ...
  describe("buildNameQuery", () => {
    ...
    it("builds exact match query when exact is true", () => {
      expect(buildNameQuery("document.txt", true)).toBe("name = 'document.txt'");
    });

    it("builds contains query when exact is false", () => {
      expect(buildNameQuery("document", false)).toBe("name contains 'document'");
    });
  ...
  describe("buildFullTextQuery", () => {
    it("builds full text search query", () => {
      expect(buildFullTextQuery("meeting notes")).toBe("fullText contains 'meeting notes'");
    });

    it("escapes special characters", () => {
      expect(buildFullTextQuery("client's report")).toBe("fullText contains 'client\\'s report'");
    });
  });
  ...
  describe("combineQueries", () => {
    ...
    it("combines multiple queries", () => {
      expect(combineQueries("'root' in parents", "name = 'doc'", "trashed = false")).toBe(
        "'root' in parents and name = 'doc' and trashed = false",
      );
    });

...
describe("utils/mimeTypes", () => {
  describe("GOOGLE_MIME_TYPES constants", () => {
    it("has correct document MIME type", () => {
      expect(GOOGLE_MIME_TYPES.DOCUMENT).toBe("application/vnd.google-apps.document");
    });

    it("has correct spreadsheet MIME type", () => {
      expect(GOOGLE_MIME_TYPES.SPREADSHEET).toBe("application/vnd.google-apps.spreadsheet");
    });

    it("has correct presentation MIME type", () => {
      expect(GOOGLE_MIME_TYPES.PRESENTATION).toBe("application/vnd.google-apps.presentation");
    });
  ...
  describe("isGoogleWorkspaceMimeType", () => {
    ...
    it("returns true for Google Sheet", () => {
      expect(isGoogleWorkspaceMimeType(GOOGLE_MIME_TYPES.SPREADSHEET)).toBe(true);
    });

    it("returns true for Google Slides", () => {
      expect(isGoogleWorkspaceMimeType(GOOGLE_MIME_TYPES.PRESENTATION)).toBe(true);
    });
  ...
  describe("getTypeFromGoogleMime", () => {
    ...
    it("returns sheet for spreadsheet MIME type", () => {
      expect(getTypeFromGoogleMime(GOOGLE_MIME_TYPES.SPREADSHEET)).toBe("sheet");
    });

    it("returns slides for presentation MIME type", () => {
      expect(getTypeFromGoogleMime(GOOGLE_MIME_TYPES.PRESENTATION)).toBe("slides");
    });

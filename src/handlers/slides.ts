...
export async function handleCreateGoogleSlides(
  ...
  const slideIds = data.slides.map(
    () => `slide_${randomUUID().substring(0, 8)}`,
  );

  // API call 3: Batch create all slides at once
  const createSlideRequests: slides_v1.Schema$Request[] = slideIds.map(
    (slideObjectId) => ({
      createSlide: {
...
export async function handleCreateGoogleSlidesShape(
  ...
  return successResponse(
    `Created ${data.shapeType} shape with ID: ${elementId}`,
  );
}
...
export async function handleGetGoogleSlidesSpeakerNotes(
  ...
  return successResponse(
    notesText.trim() || "No speaker notes found for this slide",
  );
}
...
export async function handleUpdateGoogleSlidesSpeakerNotes(
  ...
  return successResponse(
    `Successfully updated speaker notes for slide ${data.slideIndex}`,
  );
}
...
export async function handleFormatGoogleSlidesElement(
  ...
  if (data.targetType === "text") {
    ...
    if (data.backgroundColor) {
      shapeProperties.shapeBackgroundFill = toSlidesSolidFill(
    ...
    if (data.slideBackgroundColor) {
      ...
      appliedFormats.push(
        `slide background (${data.pageObjectIds!.length} slide(s))`,
      );
    }
  }
  ...
  return successResponse(
    `Applied formatting to ${targetDesc}: ${appliedFormats.join(", ")}`,
  );
}

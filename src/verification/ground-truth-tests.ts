...
async function testEmptyTrashSupportsAllDrives() {
  ...
  try {
    ...
    if (msg.includes('shared drive') || msg.includes('teamDrive')) {
      recordResult(testName, false, 'Missing supportsAllDrives causes Shared Drive failure', msg);
    } else {
...
async function testSpecialCharacterEscaping() {
  ...
  try {
    ...
    const testCases = [
      ...
      { name: 'file with "double quotes"', escaped: 'file with "double quotes"' },
    ...
    for (const tc of testCases) {
      try {
        ...
        if (msg.includes('Invalid query')) {
          recordResult(testName, false, `Query syntax error for: ${tc.name}`, msg);
          return;
        }
      }
    }
...
async function testRateLimitingBehavior() {
  ...
  try {
    ...
    if (msg.includes('429') || msg.includes('Rate Limit') || msg.includes('quota')) {
      recordResult(testName, true, undefined, `Rate limiting triggered as expected: ${msg}`);
    } else {
      recordResult(testName, false, msg);
    }
  }
      ...
      if (notesPage) {
        ...
        for (const element of pageElements) {
          if (element.shape?.shapeType === 'TEXT_BOX') {
            ...
            if (textContent) {
              log(`    Found speaker notes text box with ${textContent.length} text elements`);
            }
          }
        }
      } else {
  ...
  try {
    ...
    for (const folderName of unicodeFolderNames) {
      ...
      try {
        ...
        const msg = error instanceof Error ? error.message : String(error);
        recordResult(testName, false, `Unicode query failed for "${folderName}"`, msg);
        return;
      }
    }

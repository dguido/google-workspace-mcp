...
describe('handleMoveItem', () => {
  ...
  it('moves item successfully', async () => {
    vi.mocked(mockDrive.files.get)
      .mockResolvedValueOnce({ data: { name: 'file.txt', parents: ['parent1'] } } as never)
...
describe('handleExportFile', () => {
  ...
  it('returns error for empty fileId', async () => {
    const result = await handleExportFile(mockDrive, { fileId: '', format: 'pdf' });
    expect(result.isError).toBe(true);
  });
});
...
describe('handleStarFile', () => {
  ...
  it('stars file successfully', async () => {
    ...
    const result = await handleStarFile(mockDrive, { fileId: 'file123', starred: true });
    expect(result.isError).toBe(false);
  ...
  it('unstars file successfully', async () => {
    ...
    const result = await handleStarFile(mockDrive, { fileId: 'file123', starred: false });
    expect(result.isError).toBe(false);
  ...
  it('returns error for empty fileId', async () => {
    const result = await handleStarFile(mockDrive, { fileId: '', starred: true });
    expect(result.isError).toBe(true);
  });
});

...
describe("handleCreateGoogleSlides", () => {
  ...
  it("creates presentation successfully", async () => {
    ...
    vi.mocked(mockSlides.presentations.batchUpdate).mockResolvedValue(
      {} as never,
    );
...
describe("handleGetGoogleSlidesContent", () => {
  ...
  it("returns helpful error for file type mismatch", async () => {
    ...
    expect(result.content[0].text).toContain(
      "is not a Google Slides presentation",
    );
...
describe("handleUpdateGoogleSlidesSpeakerNotes", () => {
  ...
  it("updates speaker notes successfully when notes are empty", async () => {
    ...
    const batchUpdateCall = vi.mocked(mockSlides.presentations.batchUpdate).mock
      .calls[0][0] as {
                ...
                pageElements: [
                  {
                    ...
                    shape: {
                      text: {
                        textElements: [
                          { textRun: { content: "Existing notes" } },
                        ],
                      },
                    },
                  },

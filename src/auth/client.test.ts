...
describe("auth/client", () => {
  ...
  describe("loadCredentials", () => {
    describe("installed credentials format", () => {
      it("loads credentials from installed format", async () => {
        ...
        vi.mocked(fs.readFile).mockResolvedValue(JSON.stringify(installedCreds));
  ...
  describe("initializeOAuth2Client", () => {
    ...
    it("uses first redirect_uri as default", async () => {
      const creds = {
        installed: {
          client_id: "test-client-id",
          client_secret: "test-client-secret",
          redirect_uris: ["http://localhost:4000/callback", "http://localhost:3000/oauth2callback"],
        },
      };
    ...
    it("throws error when credentials cannot be loaded", async () => {
      ...
      await expect(initializeOAuth2Client()).rejects.toThrow("Error loading OAuth keys");
    });
  });
});

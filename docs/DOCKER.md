...
2. **Verify token location**:
   ```bash
   ls -la ~/.config/google-workspace-mcp/tokens.json
   ```
...
docker run -it \
  ...
  -v ~/.config/google-workspace-mcp/tokens.json:/config/tokens.json \
  ...
  "mcpServers": {
    "google-drive": {
      ...
      "args": [
        ...
        "-v",
        "/Users/yourname/.config/google-workspace-mcp/tokens.json:/config/tokens.json",
        "google-workspace-mcp"
      ]
    }
  }
...
-v ~/.config/google-workspace-mcp/tokens.json:/config/tokens.json:ro
```
